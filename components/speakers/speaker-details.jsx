...
import { X, Linkedin, Twitter, ExternalLink, Building2, UserX, Lock } from "lucide-react";
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-6 py-10">
          ...
          >
            ...
            <div className="w-full md:w-[42%] relative h-64 md:h-auto bg-navy-surface flex-shrink-0">
               {!REVEAL_SPEAKERS ? (
                    <div className="w-full h-full flex items-center justify-center bg-navy-deeper p-12 overflow-hidden relative">
                         <UserX size={80} className="text-gold/5 blur-[2px]" />
                         {/* Scrolling scan line for depth */}
                         <div className="absolute inset-x-0 h-[2px] bg-gold/20 animate-scan" style={{ top: '20%' }} />
            ...
            <div className="flex-1 flex flex-col p-8 md:p-12 overflow-y-auto scrollbar-thin scrollbar-thumb-gold/20 scrollbar-track-transparent">
              ...
              <div className="mb-8 relative">
                ...
                <h2 className="text-3xl md:text-5xl font-heading font-black text-white leading-tight mb-2 uppercase tracking-tighter">
                  {REVEAL_SPEAKERS ? speaker.name : `ENTITY_0x0${speaker.id} // TBA`}
                </h2>
                <div className="flex flex-col gap-2">
                  <p className="text-gold font-bold text-lg flex items-center gap-2 uppercase tracking-wide">
                    {speaker.role}
                  </p>
                  <p className="text-white-dim flex items-center gap-2 text-sm uppercase tracking-widest font-black opacity-60">
              ...
              {/* Bio Section */}
              <div className="flex-1 mb-10 overflow-hidden relative">
                <p className="text-white-dim text-xs uppercase tracking-widest font-bold mb-4 flex items-center gap-2">
                  {REVEAL_SPEAKERS ? "Biography" : "ENCRYPTED_DATA_STREAM"}
                  <span className="h-px bg-gold/20 flex-1 ml-2" />
                </p>
                
                <div className={`text-white-muted text-sm md:text-base leading-relaxed space-y-4 relative ${!REVEAL_SPEAKERS && 'opacity-20 blur-[6px] select-none pointer-events-none'}`}>
                  {speaker.bio}
                </div>

                {!REVEAL_SPEAKERS && (
                    <div className="absolute inset-x-0 top-12 bottom-0 flex flex-col items-center justify-center text-center p-6">
                         <Lock size={32} className="text-gold/40 mb-4 animate-pulse" />
                         <h4 className="text-white font-heading font-black text-xl uppercase tracking-tighter mb-2">Transmission Locked</h4>
                         <p className="text-white-dim text-xs uppercase tracking-[0.3em] max-w-xs leading-relaxed">
                            Full profile data will be synthesized upon identity confirmation. Stay tuned for the reveal.
                         </p>
                    </div>
                )}
              </div>
